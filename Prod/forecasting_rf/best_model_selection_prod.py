...
# MAGIC %run /Shared/Prod/forecasting_rf/models/arima

# COMMAND ----------

# MAGIC %run /Shared/Prod/forecasting_rf/models/lstm

# COMMAND ----------

# MAGIC %run /Shared/Prod/forecasting_rf/models/auto_ets

# COMMAND ----------

# MAGIC %run /Shared/Prod/forecasting_rf/models/croston

# COMMAND ----------

# MAGIC %run /Shared/Prod/forecasting_rf/models/trend

# COMMAND ----------

# MAGIC %run /Shared/Prod/forecasting_rf/models/linear_regression

# COMMAND ----------

# MAGIC %run /Shared/Prod/forecasting_rf/models/naive_forecast

# COMMAND ----------

# MAGIC %run /Shared/Prod/forecasting_rf/models/prophet_univariate

# COMMAND ----------

# MAGIC %run /Shared/Prod/forecasting_rf/models/randomForest_univariate

# COMMAND ----------

# MAGIC %run /Shared/Prod/forecasting_rf/models/sarima

# COMMAND ----------

# MAGIC %run /Shared/Prod/forecasting_rf/models/tbats_sktime

# COMMAND ----------

# MAGIC %run /Shared/Prod/forecasting_rf/models/xgBoost_simple

# COMMAND ----------

# MAGIC %run /Shared/Prod/forecasting_rf/models/static_ets

# COMMAND ----------

# MAGIC %run /Shared/Prod/forecasting_rf/models/MLP

# COMMAND ----------

# MAGIC %run /Shared/Prod/forecasting_rf/models/bagged_ets
...
#     global smoothing_rules, window_length_smoothing
    ...
    if smoothing_type == 'Moving Average':
        #train_data = moving_average_smoothing(train_data.copy(), window_length_smoothing)
...
## Trend logic  Giving best results comparitively 
@pandas_udf(dp3_data.schema, PandasUDFType.GROUPED_MAP)
def create_forecast_allocation_data_frame(df_in):
    ...
    df_temp['trend'][np.isinf(df_temp['trend'])] = 0 # code added to remove blanks from BRP
    df_temp['trend'][np.isnan(df_temp['trend'])] = 0 # code added to remove blanks from BRP
...
# # Previously using till 24th January 2023
# @pandas_udf(dp3_data.schema, PandasUDFType.GROUPED_MAP)
# def create_forecast_allocation_data_frame(df_in):
#     global forecast_length
#     df_in.sort_values(['period'],inplace = True)
