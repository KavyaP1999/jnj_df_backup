...
if run_mode == 'MR_forecast':
    if ingestion == 1:
        ...
        run_all_df1.repartition(128).write.mode('overwrite').parquet(working_path)
        print('ingested in run_data, run_data_all_models with run_mode MR_forecast')
...
#         db_ingestion(model_scoring_df, 'model_selection_scores', 'append')
        ...
        working_path=f"/mnt/disk/staging_data/{env}/{domain_id}/Run_id_{run_id}/{run_id}_F3_run_data_all_models.parquet"
        run_all_df1.repartition(128).write.mode('overwrite').parquet(working_path)
        print('ingested in run_data_all_models with model = dynamic')
    ...
    if ingestion == 1:
        ...
        run_all_df1 = run_all_df1.na.drop(subset=["forecast"])
        db_ingestion(run_all_df1, 'run_data_all_models', 'append')
        print('ingested in run_data_all_models with model = static')
...
#         filtered_df.repartition(128).write.mode('overwrite').parquet(folderPath)
        ...
        print('ingested in run_data with model = dynamic')
...
#         run_all_df.repartition(128).write.mode('overwrite').parquet(folderPath)
        ...
        print('ingested in run_data with model = static')
    else:
        print('Ingestion not done')
...
# MAGIC %run /Shared/Prod/forecasting/brp_product_master_generator_prod
...
# MAGIC %run /Shared/Prod/forecasting/exception_prod
# MAGIC
