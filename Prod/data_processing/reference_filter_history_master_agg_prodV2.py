...
npf_master = spark.read.jdbc(url=url, table= f"(SELECT * FROM npf_master WHERE domain_id = '{Domain}') as npf_master", properties = properties)
npf_master = npf_master.toPandas()
#npf_master.display()
...
#     npf_master_data = db_obj.execute_query(db_conn, "select " + ','.join(
#         npf_master_columns) + " from npf_master where domain_id=" + "'" + domain_id + "'")
#     npf_master_data = pd.read_csv("npf_master.csv")
...
outSchema = StructType([StructField('domain_id',StringType(),True),
                        ...
                        StructField('forecast',DecimalType(13,3),True),
...
#         reference_master = spark.read.jdbc(url=url, table= f"(SELECT * FROM reference_master WHERE run_id = '{run_id}' and domain_id = '{Domain}') as reference_master",properties = properties)
        print("reference_master counts", reference_master.toPandas().shape)
...
#             raw_run_data['key']  = raw_run_data['org_unit_id'] + '@' + raw_run_data['channel_id'] + '@' + raw_run_data['product_id']
...
#             raw_run_data.drop(columns={'key'}, inplace=True)
...
#         print(raw_run_data.shape)
    ...
    print("npf_filter counts", npf_filter.toPandas().shape)
...
# MAGIC %run /Shared/Prod/data_processing/masters_aggregation/history_master_aggregation_prodV2
