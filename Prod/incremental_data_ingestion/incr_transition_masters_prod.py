...
raw_history_master = spark.read.csv(f"/mnt/disk/staging_data/{env}/{domain_id}/incremental_staging_data/{domain_id}_raw_history_master.csv",inferSchema=True,header=True)
...
# df1 = df1.withColumn("status",when(col("status")=='Active', 1).when(col("status")=='NPI', 2).when(col("status")=='Inactive', 3).otherwise(4))
...
raw_history_master.coalesce(1).write.format("com.databricks.spark.csv").option("header","true").mode("OverWrite").save(f"/mnt/disk/staging_data/{env}/{domain_id}/incremental_staging_data/transitioned_raw_history_master_{domain_id}.csv") 
...
data_correction_master.coalesce(1).write.format("com.databricks.spark.csv").option("header","true").mode("OverWrite").save(f"/mnt/disk/staging_data/{env}/{domain_id}/incremental_staging_data/transitioned_data_correction_{domain_id}.csv") 
