...
incr_raw_hist_master = df_raw.withColumn('expost_sales', df_raw.historical_sale)

# COMMAND ----------

incr_raw_hist_master = incr_raw_hist_master.withColumnRenamed("historical_sale", "cdh")

# COMMAND ----------

#ingestion will happen in I3 state
db_ingestion(incr_raw_hist_master, 'raw_history_master', 'append')

# COMMAND ----------

raw_history_master_old = spark.read.jdbc(url=url, table= f"(SELECT * FROM raw_history_master WHERE domain_id = '{domain_id}' and  run_state = 'I5') as raw_history_master", properties = properties)

# COMMAND ----------

new_incr_raw_hist_master = raw_history_master_old.union(incr_raw_hist_master)

# COMMAND ----------

#save "new_incr_raw_hist_master" in blob storage
new_incr_raw_hist_master.coalesce(1).write.format("com.databricks.spark.csv").option("header","true").mode("OverWrite").save(f"/mnt/disk/staging_data/{env}/{domain_id}/incremental_staging_data/{domain_id}_raw_history_master.csv") 
