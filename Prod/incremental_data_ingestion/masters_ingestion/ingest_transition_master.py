...
df = spark.read.csv(f"/mnt/disk/staging_data/{env}/{domain_id}/incremental_staging_data/{domain_id}_transition_master.csv",inferSchema=True,header=True)
df = df.withColumn('org_unit_id',F.col('org_unit_id').cast(StringType())).withColumn('old_sku',F.col('old_sku').cast(StringType())).withColumn('new_sku',F.col('new_sku').cast(StringType())).withColumn('final_sku',F.col('final_sku').cast(StringType()))
...
db_ingestion(df, 'transition_final_master', 'append')
