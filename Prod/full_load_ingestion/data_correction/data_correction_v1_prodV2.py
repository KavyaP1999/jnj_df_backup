...
raw_history_master = spark.read.csv(f"/mnt/disk/staging_data/prod/{domain}/filtered_active_npi_raw_history_master_{domain}.csv",inferSchema=True,header=True)

# COMMAND ----------

raw_history_master_df = raw_history_master.select("*").toPandas()
raw_history_master_df['period'] = raw_history_master_df['period'].astype(str)

# COMMAND ----------

data_correction = spark.read.csv(f"/mnt/disk/staging_data/prod/{domain}/transitioned_data_correction_{domain}.csv",inferSchema=True,header=True)
data_correction_df = data_correction.toPandas()

# COMMAND ----------

def datacorrection(raw_history_master, new_data):
    raw_history_master['product_id'] = raw_history_master['product_id'].astype(str)
    raw_history_master['org_unit_id'] = raw_history_master['org_unit_id'].astype(str)
    raw_history_master['channel_id'] = raw_history_master['channel_id'].astype(str)
    raw_history_master['domain_id'] = raw_history_master['domain_id'].astype(str)
...
#                 values_with_UP['cdh'] = raw_history_master[raw_history_master['key']==i]['cdh'].values[0]
        ...
        if (len(values_with_period)) > 0:
            ...
            print("types of",values_with_period_temp.dtypes )
...
#             final_df_copy['key'] = final_df_copy['org_unit_id'] + '@' + final_df_copy['channel_id'] + '@' + \
#                                    final_df_copy['product_id']
#             final_df_copy.rename(columns={"historical_sale": "expost_sales"}, inplace=True)
#             raw_history_master_copy = raw_history_master_copy.merge(final_df_copy, on=['key', 'period'], how='left', indicator=False)
#             raw_history_master_copy = raw_history_master_copy[['key', 'period', 'cdh', 'expost_sales']]
#             raw_history_master_copy['domain_id'] = domain
...
#         raw_history_master_copy['expost_sales'] = raw_history_master_copy['cdh']
#         raw_history_master_copy['domain_id'] = domain
