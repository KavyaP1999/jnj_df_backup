...
transitioned_raw_history_master = spark.read.csv(f"/mnt/disk/staging_data/test/{domain}/transitioned_raw_history_master_{domain}.csv",inferSchema=True,header=True)

# COMMAND ----------

# #new changes NPF transition
...
final_data.coalesce(1).write.format("com.databricks.spark.csv").option("header","true").mode("OverWrite").save(f"/mnt/disk/staging_data/test/{domain}/filtered_active_npi_raw_history_master_{domain}.csv") 
