...
# print(raw_future_forecast_master.count(),raw_nts_master.count(),raw_past_forecast_master.count(),raw_history_master.count(),data_correction_master.count())
...
# def db_ingestion1(final, table_name, mode):
#     counts = final.count()
#     paritions = counts // 1000000 + 1
#     #default partition = 24
#     final.repartition(paritions).write.jdbc(url=url, table=table_name, mode=mode, properties=properties)

# COMMAND ----------

# db_ingestion1(final, 'npf_master', 'append')
...
raw_future_forecast_master.count()

# COMMAND ----------

raw_future_forecast_master.coalesce(1).write.format("com.databricks.spark.csv").option("header","true").mode("OverWrite").save(f"/mnt/disk/staging_data/test/{domain_id}/transitioned_raw_future_forecast_master_{domain_id}.csv") 
...
raw_nts_master.count()

# COMMAND ----------

raw_nts_master.coalesce(1).write.format("com.databricks.spark.csv").option("header","true").mode("OverWrite").save(f"/mnt/disk/staging_data/test/{domain_id}/transitioned_raw_nts_master_{domain_id}.csv")
...
raw_past_forecast_master.count()

# COMMAND ----------

raw_past_forecast_master.coalesce(1).write.format("com.databricks.spark.csv").option("header","true").mode("OverWrite").save(f"/mnt/disk/staging_data/test/{domain_id}/transitioned_raw_past_forecast_master_{domain_id}.csv") 
...
raw_history_master.coalesce(1).write.format("com.databricks.spark.csv").option("header","true").mode("OverWrite").save(f"/mnt/disk/staging_data/test/{domain_id}/transitioned_raw_history_master_{domain_id}.csv") 
...
data_correction_master = data_correction_master.na.fill(value = 0,subset = ['to_period','historical_sale'])
...
data_correction_master.count()

# COMMAND ----------

data_correction_master.coalesce(1).write.format("com.databricks.spark.csv").option("header","true").mode("OverWrite").save(f"/mnt/disk/staging_data/test/{domain_id}/transitioned_data_correction_{domain_id}.csv") 
