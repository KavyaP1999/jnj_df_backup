...
def nts_from_database(run_id, domain_id):
    df_nts_1 = spark.read.jdbc(url=url, table= f"(select * from nts_master where run_id = {run_id} and domain_id = {domain_id}) as nts_master", properties=properties)
    df_nts_1 = df_nts_1.withColumn('m1',F.col('m1').cast(FloatType())).withColumn('m2',F.col('m2').cast(FloatType())).withColumn('m3',F.col('m3').cast(FloatType()))\
    .withColumn('m4',F.col('m4').cast(FloatType())).withColumn('m5',F.col('m5').cast(FloatType())).withColumn('m6',F.col('m6').cast(FloatType()))\
    .withColumn('m7',F.col('m7').cast(FloatType())).withColumn('m8',F.col('m8').cast(FloatType())).withColumn('m9',F.col('m9').cast(FloatType()))\
    .withColumn('m10',F.col('m10').cast(FloatType())).withColumn('m11',F.col('m11').cast(FloatType())).withColumn('m12',F.col('m12').cast(FloatType()))
...
#     df = spark.read.jdbc(url=url, table="run_data", properties=properties)
#     df = df.filter((df.run_id==run_id) & (df.run_state=='F3'))
...
# run_id = 85
# forecast_length = 12
# final_df = get_decisionTree_df(run_id,forecast_length,domain)
