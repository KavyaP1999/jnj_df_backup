...
# MAGIC %run /Shared/Test/forecasting/models/arima

# COMMAND ----------

# MAGIC %run /Shared/Test/forecasting/models/auto_ets

# COMMAND ----------

# MAGIC %run /Shared/Test/forecasting/models/croston

# COMMAND ----------

# MAGIC %run /Shared/Test/forecasting/models/linear_regression

# COMMAND ----------

# MAGIC %run /Shared/Test/forecasting/models/naive_forecast

# COMMAND ----------

# MAGIC %run /Shared/Test/forecasting/models/prophet_univariate

# COMMAND ----------

# MAGIC %run /Shared/Test/forecasting/models/randomForest_univariate

# COMMAND ----------

# MAGIC %run /Shared/Test/forecasting/models/sarima

# COMMAND ----------

# MAGIC %run /Shared/Test/forecasting/models/tbats_sktime

# COMMAND ----------

# MAGIC %run /Shared/Test/forecasting/models/xgBoost_simple

# COMMAND ----------

# MAGIC %run /Shared/Test/forecasting/models/bagged_ets

# COMMAND ----------

# MAGIC %run /Shared/Test/forecasting/models/static_ets

# COMMAND ----------

# MAGIC %run /Shared/Test/forecasting/models/lstm

# COMMAND ----------

# MAGIC %run /Shared/Test/forecasting/models/MLP
...
# dbutils.widgets.text('rule_based_model_selection', '', 'rule_based_model_selection')
# dbutils.widgets.text('forecast_length', '', 'forecast_length')
# dbutils.widgets.text('test_size', '', 'test_size')
# dbutils.widgets.text('folds', '', 'folds')
# dbutils.widgets.text('model_id', '', 'model_id')
# dbutils.widgets.text('error_weight', '', 'error_weight')
# dbutils.widgets.text('me_weight', '', 'me_weight')
# dbutils.widgets.text('phase_weight', '', 'phase_weight')
# dbutils.widgets.text('best_model_selection_metric', '', 'best_model_selection_metric')
# dbutils.widgets.text('post_forecast_adj', '', 'post_forecast_adj')
# dbutils.widgets.text('smoothing_rules', '', 'smoothing_rules')
# dbutils.widgets.text('forecast_normalization', '', 'forecast_normalization')
# dbutils.widgets.text('blended_smoothing', '', 'blended_smoothing')
# dbutils.widgets.text('window_length', '', 'window_length')
# dbutils.widgets.text('system445', '', 'system445')
...
# rule_based_model_selection = ast.literal_eval(dbutils.widgets.get("rule_based_model_selection"))
...
# folds = dbutils.widgets.get("folds")
# print(folds)
# folds = ','.join(map(str, range(int(folds))))
# model_id = dbutils.widgets.get("model_id")
# model_id = str(model_id)
# error_weight = dbutils.widgets.get("error_weight")
# error_weight = float(error_weight)
# me_weight = dbutils.widgets.get("me_weight")
# me_weight = float(me_weight)
# phase_weight = dbutils.widgets.get("phase_weight")
# phase_weight = float(phase_weight)
# best_model_selection_metric = dbutils.widgets.get("best_model_selection_metric")
# post_forecast_adj = dbutils.widgets.get("post_forecast_adj")
...
# smoothing_rules = dbutils.widgets.get("smoothing_rules")
# smoothing_rules = ast.literal_eval(smoothing_rules)
# forecast_normalization = dbutils.widgets.get("forecast_normalization")
# blended_smoothing = dbutils.widgets.get("blended_smoothing")
# window_length = dbutils.widgets.get("window_length")
# window_length_smoothing = int(window_length)
# system445 = dbutils.widgets.get("system445")
...
dp3_data = spark.read.jdbc(url=url, table= f"(SELECT org_unit_id, channel_id, product_id, period, historical_sale, forecast,segmentation FROM run_data WHERE run_id = '{run_id}' and domain_id = '{domain_id}' and  run_state = 'DP3') as dp3_data", properties = properties)
...
#     dfx = dfx[['key', 'period','historical_sale','forecast']]
    return dfx
...
#     df_out = df_out.withColumn('segmenatation',lit(''))
    return df_out
...
#     print(type(df['level_shift'].unique()[0]))
...
# dp3_data_final1 = dp3_data_final.drop('key','post_forecast_adjustment','model','level_shift')
# dp3_data_final1 = dp3_data_final1.na.drop(subset=["forecast"])
...
# dp3_data_final.select(countDistinct("org_unit_id",'product_id','channel_id')).show()
...
#     db_ingestion(dp3_data_final1, 'run_data_all_models', 'append')
else:
