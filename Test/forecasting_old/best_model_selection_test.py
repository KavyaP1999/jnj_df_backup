...
# rule_based_model_selection = dbutils.widgets.get("rule_based_model_selection")
...
# window_length_smoothing = dbutils.widgets.get("window_length")
# window_length_smoothing = int(window_length_smoothing)
...
#             test_data.loc[i, 'forecast'] = sum_of_fcst // window_length_ma_6
        ...
        yhats = naive_forecast(train_data, test_size, 'M')
        yhats = yhats.astype(int)
        yhats = np.where(yhats < 0, 0, yhats)
        test_data.loc[:, 'forecast'] = yhats
        data = pd.concat([train_data, test_data, fold_data])
    elif str(model) == '18':
