...
transitioned_raw_history_master = spark.read.csv(f"/mnt/disk/staging_data/{env}/{domain}/transitioned_raw_history_master_{domain}.csv",inferSchema=True,header=True)

# COMMAND ----------

# # #new changes transition
# npf_master = spark.read.csv(f"/mnt/disk/staging_data/{env}/{domain}/transitioned_raw_npf_master_{domain}.csv",inferSchema=True,header=True) npf_master = npf_master.filter(npf_master.domain_id==domain)

# COMMAND ----------

#New code readed from transitioned_npf_master

#npf_master = spark.read.jdbc(url=url, table="npf_master", properties=properties)# old line before transition
...
final_data.coalesce(1).write.format("com.databricks.spark.csv").option("header","true").mode("OverWrite").save(f"/mnt/disk/staging_data/{env}/{domain}/filtered_active_npi_raw_history_master_{domain}.csv") 
