...
transition = spark.read.jdbc(url = url, table = f"(SELECT * FROM transition WHERE domain_id = '{domain_id}') as transition", properties = properties)
...
transitioned_df = old_new_sku_mapping(domain_id, transition_df)
