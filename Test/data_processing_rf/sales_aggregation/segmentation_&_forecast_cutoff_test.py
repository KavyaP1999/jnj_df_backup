...
#     new_prod_cut_off = int(parameters[27])                  # need to parameterize
#     model_selection = parameters[22]
    test_size = int(parameters[160])
    folds = int(parameters[162])
    new_prod_cut_off = 12 
...
#         new_prod_cut_off = 12
#     if model_selection == 'dynamic':
    new_prod_cut_off = new_prod_cut_off + test_size + folds -1  #need to parameterize
...
#     elif input_period == 'M':
#         forecastability_cutoff = 12   # line 6 to 14 will be parameterized
#         new_product_cutoff = 6
#     elif input_period == 'D':
#         forecastability_cutoff = 365
#         new_product_cutoff = 182

#     forecastability_cutoff = int(parameters[101])
#     new_product_cutoff = int(parameters[27])    # need to parameterize

    new_product_cutoff = int(parameters[101])
...
try:
    agg_dis_agg_1 = agg_dis_agg_1.withColumn('segmentation',lit(None).cast(StringType()))
    df_filtered = agg_dis_agg_1.groupby("key").apply(product_filter)
    df_filtered = df_filtered.filter((df_filtered.key != "key_deleted_forecastibility_cutoff"))
    df_filtered = df_filtered.groupby("key").apply(calculate_segmentation)    
#     df_filtered  = df_seg.groupby("key").apply(product_filter)  # need to interchange line 4,5 with 3 
#     df_filtered = df_filtered.filter((df_filtered.key != "key_deleted_forecastibility_cutoff"))
