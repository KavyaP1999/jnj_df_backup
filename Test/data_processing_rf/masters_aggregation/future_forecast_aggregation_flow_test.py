...
# readed from transition_raw_npf_master file

transitioned_future_forecast_master = spark.read.csv(f"/mnt/disk/staging_data/{env}/{domain_id}/transitioned_raw_future_forecast_master_{domain_id}.csv",inferSchema=True,header=True)
#old code for npf readed from DB
...
#npf_master = npf_master.filter(npf_master.domain_id==domain)# adding this line for testing NPF giving error with newcode
...
filtered_future_forecast_master = final_data.drop("key")
...
try:
    if ref_master_flag == 1 :
        ...
        p_file = generate_hierarchy(d_file[d_file['hierarchy_type'] == 'product'].copy(), 'product')
        c_file = generate_hierarchy(d_file[d_file['hierarchy_type'] == 'channel'].copy(), 'channel')
        o_file = generate_hierarchy(d_file[d_file['hierarchy_type'] == 'org_unit'].copy(), 'org_unit')
        ...
        filtered_future_forecast_master = filtered_future_forecast_master.filter(filtered_future_forecast_master.product_id.isin(l))
        ...
        filtered_future_forecast_master = filtered_future_forecast_master.filter(filtered_future_forecast_master.channel_id.isin(l))
        ...
        filtered_future_forecast_master = filtered_future_forecast_master.filter(filtered_future_forecast_master.org_unit_id.isin(l))
except:
    traceback.print_exc()
    10/0
...
def product_aggregator(df1, Heirarchy, product_input_level, product_output_level):
    ...
    df = df.withColumn('tf',df.tf.cast(FloatType())).withColumn('bf',df.bf.cast(FloatType()))
    df = df.groupBy("domain_id",'org_unit_id','channel_id','product_id','period').sum('tf', 'bf').withColumnRenamed('sum(tf)','tf').withColumnRenamed('sum(bf)','bf')
    return df
...
def future_forecast_agg_function(df, Heirarchy, product_output_level, channel_output_level, org_unit_output_level, product_input_level,
    ...
    df = df.withColumn("tf", f.round(df.tf, 0)).withColumn("bf", f.round(df.bf, 0)).dropDuplicates()

    print('Past Forecast Aggregation completed')
...
future_forecast_master1 = future_forecast_master1.withColumn('run_id',f.lit(run_id))
future_forecast_master1 = future_forecast_master1.drop('org_unit_id','product_id','channel_id')
future_forecast_master1 = future_forecast_master1.withColumn('tf',future_forecast_master1.tf.cast(FloatType())).withColumn('bf',future_forecast_master1.bf.cast(FloatType()))


# COMMAND ----------

if ref_master_flag == 1 :
    reference_master = spark.read.jdbc(url=url, table= f"(SELECT * FROM reference_master WHERE run_id = '{run_id}' and domain_id = '{domain_id}') as reference_master",properties = properties) #tobe deteted
    ...
    future_forecast_master1 = future_forecast_master1.filter(col('key').isin(l))

# COMMAND ----------

if ingestion_flag ==1:
    db_ingestion(future_forecast_master1 , 'future_forecast_master', 'append')
else:
