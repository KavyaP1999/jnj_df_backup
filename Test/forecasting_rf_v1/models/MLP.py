...
def mlp(data, forecast_length = 24, max_iter = 500):
    try:
        ...
        model.add(tf.keras.layers.Dropout(0.1, seed=seed_value)) # parameter
        ...
        model.add(tf.keras.layers.Dropout(0.1, seed=seed_value)) # parameter
        model.add(tf.keras.layers.Dense(step_out,activation = 'relu', kernel_initializer=initializer))#added relu here in  new changes
        model.compile(optimizer='adam', loss='mse')
        early_stop = tf.keras.callbacks.EarlyStopping(monitor='loss', min_delta=0.0001, patience=3, mode='auto')
