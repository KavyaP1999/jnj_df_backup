...
def fetch_model(url, properties):
    model = spark.read.jdbc(url=url,table=f"(select model_id, model_desc from model) as model",properties=properties)
    model = model.toPandas()
    model = dict(zip(model['model_id'],model['model_desc']))
    return model
...
def model_names(model_id):
    global model_map
    name = model_map[int(str(model_id).split('_')[0])]
    if len(str(model_id).split('_')) > 1:
        if str(model_id).split('_')[1] == 'f':
            name = name + ' Forecast Normalization'
        elif str(model_id).split('_')[1] == 's':
            name = name + ' System 445'
    return name
...
model_map = fetch_model(url, properties)
...
if run_mode == 'MR_forecast':
    if ingestion_flag == 1:
        ...
        run_data_all_model_pd = run_all_df1.toPandas()
...
if model_select == 'dynamic':
    ...
    model_scoring_copy = model_scoring_df
    model_scoring_df = model_scoring_df.withColumn('keys', concat_ws('@', model_scoring_df.key, model_scoring_df.model_id))
    model_scoring_grouped = model_scoring_df.orderBy(['key','score'], ascending=[1, 1]).groupBy('key').agg({'keys':'first'}).withColumnRenamed('first(keys)', 'keys')
...
#         save_output1(filtered_df, excel_path, 'F3')
        ...
        excel_path = f'/mnt/jnj_output_file/{env}/{domain_id}/Run_ID_{run_id}/model_selection_scores_{run_id}.xlsx'
        model_scoring_copy = model_scoring_copy.drop('metric_value_rank', 'phasing_rank', 
                                                     'mean_error_rank', 'metric_value_score', 
                                                     'phasing_score', 'mean_error_score', 'keys')
        model_scoring_copy_schema = model_scoring_copy.schema
        columns = model_scoring_copy.rdd.map(lambda x:model_names(x['model_id']), preservesPartitioning=True).collect()
        model_scoring_copy = model_scoring_copy.toPandas()
        model_scoring_copy['model_id'] = columns
        model_scoring_copy = spark.createDataFrame(model_scoring_copy, model_scoring_copy_schema)
        save_output1(model_scoring_copy, excel_path, 'Model_Scoring_Score')
...
# MAGIC %run /Shared/Test/forecasting_rf_v1/brp_product_master_generator_test
...
# MAGIC %run /Shared/Test/forecasting_rf_v1/exception_test
# MAGIC
...
run_id = int(run_id)
final_df = spark.createDataFrame(get_decisionTree_df(run_id,forecast_length,domain_id))
