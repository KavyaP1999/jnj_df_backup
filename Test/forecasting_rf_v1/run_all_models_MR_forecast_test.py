...
# Code Changes 
# 18-05-2022 - Code Reverted from new to old forecast normalization logic
...
# MAGIC %run /Shared/Test/forecasting_rf_v1/models/arima

# COMMAND ----------

# MAGIC %run /Shared/Test/forecasting_rf_v1/models/auto_ets

# COMMAND ----------

# MAGIC %run /Shared/Test/forecasting_rf_v1/models/croston

# COMMAND ----------

# MAGIC %run /Shared/Test/forecasting_rf_v1/models/trend

# COMMAND ----------

# MAGIC %run /Shared/Test/forecasting_rf_v1/models/linear_regression

# COMMAND ----------

# MAGIC %run /Shared/Test/forecasting_rf_v1/models/naive_forecast

# COMMAND ----------

# MAGIC %run /Shared/Test/forecasting_rf_v1/models/prophet_univariate

# COMMAND ----------

# MAGIC %run /Shared/Test/forecasting_rf_v1/models/randomForest_univariate

# COMMAND ----------

# MAGIC %run /Shared/Test/forecasting_rf_v1/models/sarima

# COMMAND ----------

# MAGIC %run /Shared/Test/forecasting_rf_v1/models/tbats_sktime

# COMMAND ----------

# MAGIC %run /Shared/Test/forecasting_rf_v1/models/xgBoost_simple

# COMMAND ----------

# MAGIC %run /Shared/Test/forecasting_rf_v1/models/bagged_ets

# COMMAND ----------

# MAGIC %run /Shared/Test/forecasting_rf_v1/models/static_ets

# COMMAND ----------

# MAGIC %run /Shared/Test/forecasting_rf_v1/models/lstm

# COMMAND ----------

# MAGIC %run /Shared/Test/forecasting_rf_v1/models/MLP
...
# # Old Code  issue : org.apache.spark.SparkException: Exception thrown in awaitResult:
...
def system_445(data):
    ...
    #below line old sytem445----

    #data['forecast'] = data.apply(lambda x: x['forecast'] * (15 / 13) if x['Month'] % 3 == 0 else x['forecast'] * (12 / 13) , axis=1)
    
    # below line added new system 445-----
    #data['forecast'] = data.apply(lambda x: x['forecast'] * (15 / 12) if x['Month'] % 3 == 0 else x['forecast'] * (10.5/12) , axis=1)

    # Below line added for new strong system-445 model
    data['forecast'] = data.apply(lambda x: x['forecast'] * (19 / 12) if x['Month'] % 3 == 0 else x['forecast'] * (8.5 / 12) , axis=1)
...
def create_forecast_allocation_data_frame(df_in):
    ...
    df_temp['trend'][np.isinf(df_temp['trend'])] = 0 # recently added line to remove blanks in BRP
    df_temp['trend'][np.isnan(df_temp['trend'])] = 0 # recently added line to remove blanks in BRP
    
    df_temp['forecast'] = (1 + df_temp['trend']) * df_temp['forecast']
    
    df_temp['forecast'] = df_temp['forecast'].round()
    df_temp.loc[df_temp['forecast'] < 0,'forecast'] = 0 # newly added to remove negatives

    #df_temp[df_temp['forecast'] < 0]['forecast'] = 0    # intentionally commented for checking -ve forecast values present or not
...
#     df_temp['allocations'] = df_temp['allocations'].div(historical_sum)
#     df_temp['forecast'] = df_temp['allocations'].mul(forecast_sum)
#     segmentation = df_temp['segmentation'].drop_duplicates().tolist()[0]
#     yhat_mean = yhat[0]
#     damping_value = 0.95
#     yhat_damp = [damping_value]
#     df_temp['trend'] = yhat
#     for i in range(forecast_length - 1):
#         yhat_damp.append(yhat_damp[-1] * damping_value)
#     df_temp['trend'] = (df_temp['trend'] - yhat_mean)/np.abs(yhat_mean)
#     df_temp['trend_damp'] = yhat_damp
#     df_temp['trend'] = df_temp['trend'] * df_temp['trend_damp']
#     if segmentation in ("Regular",'NewProductRegular'):
#         df_temp['forecast'] = (1 + df_temp['trend']) * df_temp['forecast']
#         df_temp['forecast'] = df_temp['forecast'].round()
#     df_temp.loc[df_temp['forecast'] < 0, 'forecast'] = 0
#     dfx = pd.concat(([dfx, df_temp]), axis=0, ignore_index=True)
#     dfx = dfx.drop(['allocations','org_unit_id','product_id','channel_id','allocated_period','trend','trend_damp'], axis=1)
#     return dfx
