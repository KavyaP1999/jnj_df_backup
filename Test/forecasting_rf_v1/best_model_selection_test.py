...
no_of_folds = int(parameters[162])
...
blended_smoothing = eval(parameters[155])
window_length_smoothing = int(parameters[92])
# window_length_static = 3
system445 = eval(parameters[156])
# alpha = 0
# beta = 0
# gamma = 0.3
...
def system_445(data):
    ...
    test_data['forecast'] = test_data.apply(lambda x: x['forecast'] * (15 / 12) if x['Month'] % 3 == 0 else x['forecast'] * (10.5 / 12) , axis=1)
...
def create_forecast_allocation_data_frame(df_in):
    ...
    df_temp['trend'][np.isinf(df_temp['trend'])] = 0 # recent changes to remove Blanks in BRP
    df_temp['trend'][np.isnan(df_temp['trend'])] = 0 # recent changes to remove Blanks in BRP
    
    df_temp['forecast'] = (1 + df_temp['trend']) * df_temp['forecast']
    df_temp['forecast'] = df_temp['forecast'].round()
    df_temp.loc[df_temp['forecast'] < 0,'forecast'] = 0 # recent added to remove negatives values
...
#     dfx = pd.concat(([dfx, df_temp]), axis=0, ignore_index=True)
#     dfx = dfx.drop(['allocations','org_unit_id','product_id','channel_id','allocated_period','trend','trend_damp','fold'], axis=1)
#     return dfx
...
#     df_temp['forecast'] = df_temp['forecast'].round()
#     dfx = pd.concat(([dfx, df_temp]), axis=0, ignore_index=True)
#     dfx = dfx.drop(['allocations','org_unit_id','product_id','channel_id','allocated_period','fold'], axis=1)
#     return dfx
...
def best_model_selection(data):
    ...
    data['rank_phase'] = int(no_of_folds) - data['phase'] + 1
