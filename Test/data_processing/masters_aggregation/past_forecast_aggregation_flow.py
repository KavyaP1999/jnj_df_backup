...
transitioned_past_forecast_master = spark.read.csv(f"/mnt/disk/staging_data/test/{domain_id}/transitioned_raw_past_forecast_master_{domain_id}.csv",inferSchema=True,header=True)

# COMMAND ----------

#old code for NPF readed from databse
#npf_master = spark.read.jdbc(url=url, table="npf_master", properties=properties)
# New code readed from CSV
...
#npf_master = npf_master.filter(npf_master.domain_id==domain_id) #old line before npf
npf_master = npf_master.filter(npf_master.domain_id==domain) # new line after NPF
...
filtered_past_forecast_master = final_data.toPandas()
...
try:
    ...
    if ref_master_flag == 1 :
        ...
        filtered_past_forecast_master = filtered_past_forecast_master[filtered_past_forecast_master['product_id'].isin(hierarchy_master[f'product_{product_input_level}'])]
        hierarchy_master = c_file[c_file[f'channel_{channel_forecast_level}'].isin(reference_master_split['channel_id'])]
        filtered_past_forecast_master = filtered_past_forecast_master[filtered_past_forecast_master['channel_id'].isin(hierarchy_master[f'channel_{channel_input_level}'])]
        hierarchy_master = o_file[o_file[f'org_unit_{org_unit_forecast_level}'].isin(reference_master_split['org_unit_id'])]
        filtered_past_forecast_master = filtered_past_forecast_master[filtered_past_forecast_master['org_unit_id'].isin(hierarchy_master[f'org_unit_{org_unit_input_level}'])]
except:
    traceback.print_exc()
    10/0
#     pipeline_flag = 1
...
def product_aggregator(df1, Heirarchy, product_input_level, product_output_level):
    ...
    df['bf_m00'] = df['bf_m00'].astype(float)
    df['bf_m01'] = df['bf_m01'].astype(float)
    df['bf_m02'] = df['bf_m02'].astype(float)
    df['bf_m03'] = df['bf_m03'].astype(float)
    df['tf_m00'] = df['tf_m00'].astype(float)
    df['tf_m01'] = df['tf_m01'].astype(float)
    df['tf_m02'] = df['tf_m02'].astype(float)
    df['tf_m03'] = df['tf_m03'].astype(float)
    df = df.groupby(["domain_id",'org_unit_id','channel_id','product_id','period'])['bf_m00', 'bf_m01','bf_m02','bf_m03','tf_m00','tf_m01','tf_m02','tf_m03'].sum().reset_index()
#     df = df[['domain_id','period','bf_m00', 'bf_m01','bf_m02','bf_m03','tf_m00','tf_m01','tf_m02','tf_m03']]
    return(df)
...
def past_forecast_agg_function(df1, Heirarchy, product_output_level, channel_output_level, org_unit_output_level, product_input_level=0,
    ...
    df = df[['domain_id', 'key', 'period', 'bf_m00', 'bf_m01', 'bf_m02', 'bf_m03', 'tf_m00', 'tf_m01', 'tf_m02','tf_m03']]
    df['bf_m00'] = np.round(df['bf_m00'], 0)
    df['bf_m01'] = np.round(df['bf_m01'], 0)
    df['bf_m02'] = np.round(df['bf_m02'], 0)
    df['bf_m03'] = np.round(df['bf_m03'], 0)
    df['tf_m00'] = np.round(df['tf_m00'], 0)
    df['tf_m01'] = np.round(df['tf_m01'], 0)
    df['tf_m02'] = np.round(df['tf_m02'], 0)
    df['tf_m03'] = np.round(df['tf_m03'], 0)
...
# past_forecast_master1.display()

# COMMAND ----------

# run_id = 15
past_forecast_master1['run_id'] = run_id
...
if ref_master_flag == 1 :
    ...
    past_forecast_master1 = past_forecast_master1[past_forecast_master1['key'].isin(reference_master['key'])]
...
outSchema = StructType([StructField('domain_id',StringType(),True),
                       ...
                       StructField('bf_m00',FloatType(),True),
                       StructField('bf_m01',FloatType(),True),
                       StructField('bf_m02',FloatType(),True),
                       StructField('bf_m03',FloatType(),True),
                       StructField('tf_m00',FloatType(),True),
                       StructField('tf_m01',FloatType(),True),
                       StructField('tf_m02',FloatType(),True),
                       StructField('tf_m03',FloatType(),True),
                       StructField('run_id',StringType(),True)])

# COMMAND ----------

final_data3 = spark.createDataFrame(data=past_forecast_master1, schema=outSchema)

# COMMAND ----------

db_ingestion(final_data3, 'past_forecast_master', 'append')
