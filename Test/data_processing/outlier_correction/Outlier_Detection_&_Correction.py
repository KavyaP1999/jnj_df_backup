...
# run_data_I5_stage = spark.read.jdbc(url=url, table= f"(SELECT * FROM run_data WHERE run_id = '{run_id}' and domain_id = '{domain_id}' and  run_state = 'I5') as run_data", properties = properties)
working_path=f"/mnt/disk/staging_data/{env}/{domain_id}/Run_id_{run_id}/{run_id}_I5_run_data.parquet"
run_data_I5_stage = spark.read.format('parquet').load(working_path,inferSchema=True)
...
# def insert_data(data,url,properties):
    
#     data.write.jdbc(url=url, table= "run_data", mode="append", properties = properties)
...
def moving_average_outliers(df: DataFrame):
    ...
    if df['historical_sale'].sum()==0:
        ...
        df_x = df
        #df_x = df_x.sort_values(['org_unit_id', 'channel_id', 'product_id', 'period'])
        ...
        #mov_avg = wheighted_moving_average_smoothing(df_e)
...
try:
    ...
    if outlier_detection_choice == 'ZScore': 
        ...
        #df_final.write.jdbc(url=url, table= "run_data", mode="append", properties = properties)
        
        #bulk Ingestion in DB at DP1
        #insert_data(df_final,url,properties)
        #db_ingestion(df_final, 'run_data', 'append')
        
        #parameters = run_parameter_data(run_id,url,properties)
        #status = parameters[5]
        #update_flag(status,url,properties)
...
#     excel_path = f'/mnt/jnj_output_file/{env}/{domain_id}/Run_ID_{run_id}/DP1_{run_id}.xlsx'
#     save_output1(df_final,excel_path,'DP1')
#     parameters = run_parameter_data(run_id,url,properties)
#     status = parameters[5]
...
# df_final_tmp = df_final_tmp.withColumn("key",concat_ws("~","org_unit_id","channel_id","product_id"))
# df_final_tmp = df_final_tmp.withColumn("org_unit_id", df_final_tmp["org_unit_id"].cast('string'))
# df_final_tmp.select(countDistinct("key")).show()
