...
def future_discontinuous(df1,data):
    #df1 = df
    list_dis = []
    for i in range(len(df1['key'])):
        for j in range(len(data)):
            if df1['key'][i] == data['key'][j] and data['period'][j] < df1['period'][i]:
                list_dis.append(df1['period'][i])
    return list_dis

# COMMAND ----------

def f5_future_discontinuous(df, data):
#     try:
    print(df)
    print(data)
    key_list = []
    df['key'] = df['org_unit_id'].astype(str) + '@' + df['channel_id'].astype(str)+ '@' + df['product_id'].astype(str)
    original_data = df
    temp = pd.DataFrame(columns = df.columns)
    l = list(data['key'])
    new_df = df[df.key.isin(l)]
    new_df.reset_index(drop=True,inplace = True)#discontinuous data
    print(new_df)
    if new_df.shape[0]>0:                
        df = new_df
        for k in np.unique(df['key']):
            df_1 = df.loc[(df['key']==k)]
            df_1.reset_index(drop=True,inplace = True)
            list_dis = future_discontinuous(df_1, data)
            new_df = df_1[~df_1.period.isin(list_dis)]
            new_df1 = df_1[df_1.period.isin(list_dis)]
            print('new_df')
            print(new_df.head(2))
            new_df1['forecast']=0
            final_df = new_df.append(new_df1)
            print('final_df')
            print(final_df.head(2))
            temp = pd.concat((temp, final_df), axis=0, ignore_index=True)
            print('temp')
            print(temp.head(2))
            print(temp.columns)
            
    key_list.append(temp.key.unique())
    res = [list(item) for item in key_list]
    key_list = np.array(res).flatten()
    final1 = original_data[~original_data.key.isin(key_list)]
    final = final1.append(temp)
...
#     db_ingestion(df_final, 'raw_disag_data', 'append')
    ...
    if future_discontinue_master.shape[0] == 0:
        print("future discontinue master is empty.\n future discontinue master is not applied")

    if future_discontinue_master.shape[0] > 0:
        future_discontinue_master = future_discontinue_master[future_discontinue_master['domain_id'] == domain_id]
        df_final = f5_future_discontinuous(df_final.toPandas(), future_discontinue_master)
        df_final = spark.createDataFrame(df_final)
