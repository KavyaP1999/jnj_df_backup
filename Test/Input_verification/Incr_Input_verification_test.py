...
try:    
    ...
    blobfile = []
    generator = blob_service.list_blobs('customerdata', prefix=f"{env}/Incremental_Load/{domain_id}/", delimiter="")
...
try:
    df = spark.read.csv(f"/mnt/customerdata/{env}/Incremental_Load/{domain_id}/past_forecast_master.csv",inferSchema=True,header=True)
    df_col =df.columns
...
try:
    df = spark.read.csv(f"/mnt/customerdata/{env}/Incremental_Load/{domain_id}/nts_master.csv",inferSchema=True,header=True)
    df_col =df.columns
...
try:
    df = spark.read.csv(f"/mnt/customerdata/{env}/Incremental_Load/{domain_id}/data_correction.csv",inferSchema=True,header=True)
    df_col =df.columns
...
# DBTITLE 1,npf_master column and status check
try:
...
try:
    df = spark.read.csv(f"/mnt/customerdata/{env}/Incremental_Load/{domain_id}/future_discontinue_master.csv",inferSchema=True,header=True)
    df_col =df.columns
...
#         df = spark.read.csv(f"/mnt/customerdata/{env}/Incremental_Load/{domain_id}/{names}.csv",inferSchema=True,header=True)
...
try:    
    ...
    error = f'sales_master has two parent_values for one hierarchy value in {column}'
    error_log = np.append(error_log,error)
...
try:    
    ...
    error = f'sales_master has two or more description in {column}'
    error_log = np.append(error_log,error)
