...
import pyspark.sql.functions as F # new line added
...
# New prod code
raw_history_master = spark.read.csv(f"/mnt/disk/staging_data/{env}/{domain}/incremental_staging_data/filtered_active_npi_raw_history_master_{domain}.csv",inferSchema=True,header=True)
raw_history_master = raw_history_master.withColumn('org_unit_id',F.col('org_unit_id').cast(StringType())).withColumn('product_id',F.col('product_id').cast(StringType()))
...
# New prodcode
data_correction = spark.read.csv(f"/mnt/disk/staging_data/{env}/{domain}/incremental_staging_data/transitioned_data_correction_{domain}.csv",inferSchema=True,header=True)
data_correction = data_correction.withColumn('org_unit_id',F.col('org_unit_id').cast(StringType())).withColumn('product_id',F.col('product_id').cast(StringType())).withColumn('historical_sale',F.col('historical_sale').cast(FloatType()))
