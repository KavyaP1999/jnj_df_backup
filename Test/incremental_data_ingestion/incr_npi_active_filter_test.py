...
transitioned_raw_history_master = spark.read.csv(f"/mnt/disk/staging_data/{env}/{domain}/incremental_staging_data/transitioned_raw_history_master_{domain}.csv",inferSchema=True,header=True)

# COMMAND ----------

#OLD code readed from transitioned_npf_master

#npf_master = spark.read.jdbc(url=url, table="npf_master", properties=properties)
# New code NPF below one line
...
final_data.coalesce(1).write.format("com.databricks.spark.csv").option("header","true").mode("OverWrite").save(f"/mnt/disk/staging_data/{env}/{domain}/incremental_staging_data/filtered_active_npi_raw_history_master_{domain}.csv") 
